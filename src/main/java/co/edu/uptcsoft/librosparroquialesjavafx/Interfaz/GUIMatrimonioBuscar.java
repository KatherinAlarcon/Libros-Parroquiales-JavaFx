...
public class GUIMatrimonioBuscar extends Application {
    ...
    private ControlMatrimonio controlMatrimonio;
    ArrayList<Matrimonio> listaMatrimonio;// Agregado

    @Override
    public void start(Stage stage) {

        controlMatrimonio = new ControlMatrimonio();
        listaMatrimonio = controlMatrimonio.getListaMatrimonio();
    ...
    private GridPane createSearchPanel() {
        ...
        Label searchLabel = new Label("Buscar/Matrimonio");
    ...
    private TableView<Object[]> createTable() {
        ...
        table.getColumns().addAll(numeroColumn, nombreColumn, libroColumn);
        table.setItems(data);
        ...
        table.setOnMouseClicked(event -> {
            if (event.getClickCount() == 2) { // Doble clic
                ...
                if (selectedRow != null) {
                    ...
                    abrirEdicionMatrimonio(stage, numero);
                }
            }
        });
        return table;
    ...
    private void buscar() {
        ...
        for (Matrimonio matrimonio : listaMatrimonio ) {
            boolean nombreCoincide = nombre.isEmpty() || matrimonio.getNombreNovio().toLowerCase().contains(nombre);
            boolean apellidoCoincide = apellido.isEmpty() || matrimonio.getApellidoNovio().toLowerCase().contains(apellido);

            if (nombreCoincide && apellidoCoincide) {
                data.add(new Object[]{
                        matrimonio.getNumero(),
                        matrimonio.getNombreNovio() + " " + matrimonio.getApellidoNovio(),
                        matrimonio.getLibro()
                });
            }
        }
    ...
    private void cargarTodosLosDatos() {
        data.clear();
        for (Matrimonio confirmacion : listaMatrimonio) {
            data.add(new Object[]{
                    confirmacion.getNumero(),
                    confirmacion.getNombreNovio() + " " + confirmacion.getApellidoNovio(),
                    confirmacion.getLibro()
            });
        }
    }

    // In GUIMatrimonioBuscar.java, modify the abrirEdicionMatrimonio method

    private void abrirEdicionMatrimonio(Stage currentStage, String numero) {
        try {
            GUIMatrimonio editor = new GUIMatrimonio();
            Stage newStage = new Stage();
            editor.start(newStage);
            editor.initData2(numero); // Call initData2 after start
            currentStage.close();
        } catch (Exception e) {
            e.printStackTrace();
        }
