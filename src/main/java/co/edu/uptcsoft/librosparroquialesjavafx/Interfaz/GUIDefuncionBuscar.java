...
public class GUIDefuncionBuscar extends Application {
    ...
    private ControlDefuncion controlDefuncion;
    ArrayList<Defuncion> listaDefuncion;

    @Override
    public void start(Stage stage) {

        controlDefuncion = new ControlDefuncion();
        listaDefuncion = controlDefuncion.getListaDefuncion();
    ...
    private GridPane createSearchPanel() {
        ...
        Label searchLabel = new Label("Buscar/Defuncion");
    ...
    private TableView<Object[]> createTable() {
        ...
        table.setOnMouseClicked(event -> {
            if (event.getClickCount() == 2) { // Doble clic
                ...
                if (selectedRow != null) {
                    abrirEdicionDefuncion(stage, selectedRow);
                }
            }
        });
        return table;
    ...
    private void buscar() {
        ...
        for (Defuncion defuncion : listaDefuncion) {
            boolean nombreCoincide = nombre.isEmpty() || defuncion.getNombres().toLowerCase().contains(nombre);
            boolean apellidoCoincide = apellido.isEmpty() || defuncion.getApellidos().toLowerCase().contains(apellido);

            if (nombreCoincide && apellidoCoincide) {
                data.add(new Object[]{
                        defuncion.getNumero(),
                        defuncion.getNombres() + " " + defuncion.getApellidos(),
                        defuncion.getLibro()
                });
            }
        }
    ...
    private void cargarTodosLosDatos() {
        data.clear();
        for (Defuncion confirmacion : listaDefuncion) {
            data.add(new Object[]{
    ...
    // In GUIMatrimonioBuscar.java, modify the abrirEdicionDefuncion method

    private void abrirEdicionDefuncion(Stage currentStage, Object[] selectedRow) {
        try {
            GUIDefuncion editor = new GUIDefuncion();
            Stage newStage = new Stage();
            editor.start(newStage);
            editor.initData(selectedRow); // Pasar los datos seleccionados
            currentStage.close();
        } catch (Exception e) {
            e.printStackTrace();
        }
